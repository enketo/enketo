...
describe('evalInfix()', () => {
  describe('booleans', () => {
    ...
    describe('RelationalExpr', () => {
      ...
      singleOpTestsFor('!=', ...SIMPLE_EQUALITY.map(([lhs, rhs, areEqual]) => [lhs, rhs, !areEqual]));
...
function specifiedOpTestsFor(...testCases) {
  testCases.forEach(([ lhs, op, rhs, expected ]) => testCaseFor(lhs, op, rhs, expected));
}

function singleOpTestsFor(op, ...testCases) {
  describe(op, () => testCases.forEach(([ lhs, rhs, expected ]) => testCaseFor(lhs, op, rhs, expected)));
}
